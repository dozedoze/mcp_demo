...
// 辅助函数：确保测试器已初始化
function ensureTesterInitialized() {
    if (!tester) {
...
// 注册工具：导航到指定页面
server.registerTool("navigate_to_page", {
    title: "导航页面",
    description: "导航到指定页面",
    inputSchema: {
...
// 注册工具：查看商品详情
server.registerTool("view_product_detail", {
    title: "查看商品详情",
...
// 注册工具：选择商品选项
server.registerTool("select_product_options", {
    title: "选择商品选项",
...
// 注册工具：进入结账流程
server.registerTool("proceed_to_checkout", {
    title: "进入结账",
    description: "进入结账流程",
}, async () => {
    const t = ensureTesterInitialized();
...
// 注册工具：填写收货地址
server.registerTool("fill_shipping_address", {
    title: "填写收货地址",
...
// 注册工具：运行完整测试流程
server.registerTool("run_test_flow", {
    title: "运行完整测试流程",
    description: "运行完整的测试流程：浏览 -> 搜索 -> 查看商品 -> 加购",
    inputSchema: {
