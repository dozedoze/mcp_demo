...
export class LilySilkTester {
  ...
  constructor(baseUrl: string = "https://www.lilysilk.com/us") {
    ...
    if (!this.browser) {
      ...
      for (const channel of channels) {
        try {
          ...
          this.browser = await chromium.launch({
            headless,
            channel: channel as "chrome" | "msedge" | "chromium",
            args: ["--no-sandbox", "--disable-setuid-sandbox", "--disable-dev-shm-usage"],
          });
          break;
        } catch {
...
`;
   ...
   * 搜索商品
   */
  async searchProduct(keyword: string): Promise<string> {
    this.ensurePageInitialized();
   ...
   * 浏览商品分类
   */
  async browseCategory(category: string = 'women'): Promise<string> {
    this.ensurePageInitialized();
   ...
   * 查看商品详情
   */
  async viewProductDetail(productIndex: number = 0): Promise<string> {
    this.ensurePageInitialized();
   ...
   * 选择商品选项
   */
  async selectProductOptions(options: ProductOptions): Promise<string> {
    this.ensurePageInitialized();
   ...
   * 填写收货地址
   */
  async fillShippingAddress(address: ShippingAddress): Promise<string> {
    this.ensurePageInitialized();
   ...
   * 截取屏幕截图
   */
  async takeScreenshot(filename: string = "screenshot", fullPage: boolean = false): Promise<string> {
    this.ensurePageInitialized();
   ...
   * 等待元素出现
   */
  async waitForElement(selector: string, timeout: number = 30000): Promise<string> {
    this.ensurePageInitialized();
   ...
   * 点击元素
   */
  async clickElement(selector?: string, text?: string): Promise<string> {
    this.ensurePageInitialized();
   ...
   */
  async runTestFlow(
    testAddress?: Partial<ShippingAddress>
  ): Promise<string> {
    this.ensurePageInitialized();
